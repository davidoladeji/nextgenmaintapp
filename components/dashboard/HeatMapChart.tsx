...
export default function HeatMapChart({ components, onFilterClick }: HeatMapChartProps) {
  const getRPNColor = (rpn: number) => {
    if (rpn > 150) return 'bg-red-500 hover:bg-red-600';
    if (rpn > 100) return 'bg-orange-500 hover:bg-orange-600';
    if (rpn > 70) return 'bg-yellow-500 hover:bg-yellow-600';
    return 'bg-green-500 hover:bg-green-600';
  };

  const getRPNTextColor = (rpn: number) => {
    if (rpn > 150) return 'text-red-800';
    if (rpn > 100) return 'text-orange-800';
    if (rpn > 70) return 'text-yellow-800';
  ...
  // Show only top 8 components for better visualization
    ...
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-600">Component failure modes by risk level</p>
        </div>
      ...
      <div className="space-y-3">
        {topComponents.length === 0 ? (
          ...
          topComponents.map((component) => (
            <div key={component.id} className="border border-gray-200 rounded-lg p-3 hover:shadow-md transition-shadow">
              ...
              <div className="flex flex-wrap gap-2">
                {component.failureModes && component.failureModes.length > 0 ? (
                  component.failureModes.slice(0, 10).map((fm) => {
                  ...
                  <div className="text-xs text-gray-400 italic py-1">No failure modes</div>
                )}
                {component.failureModes && component.failureModes.length > 10 && (
                  <div className="px-3 py-2 rounded bg-gray-200 text-gray-600 text-xs font-medium">
