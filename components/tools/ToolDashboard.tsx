...
import { useAuth, useOrganization, useUI } from '@/lib/store';
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-slate-400">Loading tools...</p>
        </div>
      </div>
    );
  }
            ...
            {projects.slice(0, 8).map((project) => {
              ...
              return (
                <button
                  ...
                  className="group bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden hover:border-primary-400 dark:hover:border-primary-500 hover:shadow-md hover:scale-[1.02] transition-all duration-200 text-left"
                >
                  {/* Preview/Thumbnail area */}
                  <div className="aspect-video bg-gray-50 dark:bg-slate-900 flex items-center justify-center border-b border-gray-200 dark:border-slate-700">
                    <IconComponent className="w-12 h-12 text-gray-300 dark:text-slate-600" />
                  </div>
                    ...
                    <div className="flex items-start space-x-2 mb-2">
                      <div className="flex-shrink-0 mt-0.5">
                        <div className="w-5 h-5 rounded bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center">
                          <IconComponent className="w-3 h-3 text-primary-600 dark:text-primary-400" />
                        </div>
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-medium text-gray-900 dark:text-slate-100 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors truncate">
                          {project.name}
                        </h3>
                      </div>
                    </div>
